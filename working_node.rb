		...
		read.each do |socket|
				...
				if message != nil
					...
					when "EDGEB"
						...
						# STDERR.puts "Symmetric edge from " + $hostname + " to " + node_name
...
def dumptable(cmd)
	...
	# cost_string += "\000"
	#
	# STDERR.puts "cost_string from " + $hostname + ": " + cost_string
		...
		if !visited.include? current_node
			...
			if current_node != $hostname
				if $nodes[current_node]["SOCKET"] != nil
					...
					STDERR.puts "COST message: COST #{$hostname}\000"
					STDERR.puts "DST_IP: " + $nodes[current_node]["IP"].to_s
					STDERR.puts "DST_PORT: " + $nodes[current_node]["PORT"].to_s
					...
					STDERR.puts "COST message sent to " + current_node + " at " + socket.to_s
